...
// For complicated method samples.
...
private:
    virtual void EmptyEcho(google::protobuf::RpcController* controller,
                           const poppy::EmptyRequest* request,
                           poppy::EmptyResponse* response,
                           google::protobuf::Closure* done) {
        ++m_request_count;
        LOG(INFO) << "An empty request and empty response!";
        g_total_request_count++;
        done->Run();
    }

    virtual void EmptyParamEcho(google::protobuf::RpcController* controller,
                                const poppy::EmptyRequest* request,
                                EchoResponse* response,
                                google::protobuf::Closure* done) {
        ++m_request_count;
        LOG(INFO) << "An empty parameter request!";
        response->set_user("empty");
        response->set_message("empty parameter echo");
        g_total_request_count++;
        done->Run();
    }

    virtual void EmptyReturnEcho(google::protobuf::RpcController* controller,
                                 const EchoRequest* request,
                                 poppy::EmptyResponse* response,
                                 google::protobuf::Closure* done) {
        ++m_request_count;
        LOG(INFO) << request->user() << ": " << request->message();
        g_total_request_count++;
        done->Run();
    }
