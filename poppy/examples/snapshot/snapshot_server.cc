...
private:
    ...
    virtual void QuerySnapshot(google::protobuf::RpcController* controller,
        ...
        LOG(INFO) << "query snapshot request: docid=" << request->docid()
...
int main(int argc, char** argv)
{
    ...
    while (!g_quit) {
        ...
        fprintf(stderr, "add request rate=%d, query request rate=%d, total request=%d\n",
                snapshot_service->GetAndClearAddRequestCount(),
                snapshot_service->GetAndClearQueryRequestCount(),
                g_total_request_count);
    }
