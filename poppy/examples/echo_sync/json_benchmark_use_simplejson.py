...
def PrintDebugInfo(url, request, response):
    ...
    print "message = " + request["message"].decode("utf-8").encode(TTY_ENCODING)
    print "\nReceive json response:"
    print "user    = " + response["user"]
    print "message = " + response["message"].decode("utf-8").encode(TTY_ENCODING)
    print
...
def main():
    ...
    url = "/rpc/rpc_examples.EchoServer.EchoMessage"
    ...
    while total_count < int(sys.argv[1]):
        try:
            data = {}
            data["user"] = "ericliu"
            data["message"] = u'hello,中国人'.encode("utf-8")
