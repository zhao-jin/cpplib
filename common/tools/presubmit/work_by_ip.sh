...
{
    cat >&2 <<END
Usage: $1 <ip> <user> <passwd> <cmd> <parameters>
功能:在远程机器上执行一预定义类型的命令
cmd 有以下几种，后面分别是相应的参数
cmdline,后面接在远程机器执行的命令语句,该命令在远程机器运行一行命令，本地显示结果，如$1 cmdline \"free | tail -n 1 | awk '{print $3}'\"
cmdbat,后面接在远程机器需要执行的文件名字，该命令在远程机器运行一个文件,带显式路径则直接执行该文件，否则到某个默认目录执行
copyto,后面接本机的文件名字和远程路径，该命令将本地文件复制到远程目录
copyfrom,后面接远程机器的文件名字和本地路径，该命令将远程文件复制到本地
END
    exit 1
}
...
cmdline()
{
    ...
    echo "ip:"$ip
    eval "$expscript ssh $user $passwd  $ip \"$cmd\" a.out"
}
...
copyto()
{
    ...
    #echo "copying"
    eval "$expscript scp $user $passwd $ip $1 $2 "
    #sleep 1
}
...
{
    name=`basename $1`
    eval "$expscript scpfrom $user $passwd $ip $1 ./ "
    #mv ./$name ./${name}"."$tail
